  ...
  public void testBug42164() {
    ...
    vm1.invoke(new SerializableCallable() {
      public Object call() throws Exception {
        ...
        Region pr = getCache().getRegion(PR_REG_NAME);
        for (int i=0; i<MAX_ENTRIES; i++) {
          assertEquals("value"+i, r.putIfAbsent(i, "value"));
          assertEquals("value"+i, pr.putIfAbsent(i, "value"));
        }
        for (int i=0; i<MAX_ENTRIES; i++) {
          assertFalse(r.replace(i, "value", "newValue"));
          assertFalse(pr.replace(i, "value", "newValue"));
        }
        for (int i=MAX_ENTRIES+1; i<MAX_ENTRIES*2; i++) {
          assertNull(r.replace(i, "value"+i));
          assertNull(pr.replace(i, "value"+i));
        }
        for (int i=MAX_ENTRIES+1; i<MAX_ENTRIES*2; i++) {
          assertFalse(r.remove(i, "value"+i));
          assertFalse(pr.remove(i, "value"+i));
        }
        return null;
      }
    });
