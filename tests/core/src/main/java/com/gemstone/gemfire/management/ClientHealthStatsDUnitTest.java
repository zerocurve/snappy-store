...
import com.gemstone.gemfire.cache.CacheRegionClearStatsDUnitTest;
...
public class ClientHealthStatsDUnitTest extends DistributedTestCase {
  ...
  public void testClientHealthStats() throws Exception {
    ...
    // Commented out following call to workaround bug 46736
  ...
  public Cache createClientCache(String host, Integer port1) throws Exception {
    ...
    PoolImpl p = (PoolImpl) PoolManager.createFactory().addServer(host,
        ...
        .setSubscriptionEnabled(true).setStatisticInterval(1000).create(
        ...
        try {
          ...
          getLogWriter().info(
              "<ExpectedString> ClientId of the Server is  " + clientId
                  + "</ExpectedString> ");
          ClientHealthStatus clientStatus = bean.showClientStats(clientId);
          getLogWriter().info(
              "<ExpectedString> ClientStats of the Server is  " + clientStatus
                  + "</ExpectedString> ");
          ...
          for(ClientHealthStatus status : clientStatuses){
            getLogWriter().info(
                "<ExpectedString> ClientStats of the Server is  " + status
                    + "</ExpectedString> ");
          ...
          assertEquals(2,dsBean.getNumSubscriptions());
  ...
  protected void registerInterest(final VM vm) {
    SerializableRunnable put = new SerializableRunnable("registerInterest") {
      public void run() {
        try {
          Cache cache = GemFireCacheImpl.getInstance();
          Region r1 = cache.getRegion(Region.SEPARATOR + REGION_NAME);
          ...
          fail("failed while register Interest", ex);
        }
      }
