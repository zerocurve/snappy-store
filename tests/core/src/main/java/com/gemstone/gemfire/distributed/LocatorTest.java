...
public class LocatorTest extends DistributedTestCase {
  ...
  /**
   * Creates a new <code>LocatorTest</code>
   */
  ...
  public void tearDown2() {
    ...
    if (port1 > 0) {
      deleteStateFile(port1);
    }
    if (port2 > 0) {
      deleteStateFile(port2);
    }
  }
  ...
  /**
   * GemFireXD uses a colocated locator in a dm-type=normal VM.  This tests that
...
//        boolean setting = JGroupMembershipManager.DEBUG_JAVAGROUPS;
        try {
...
//          JGroupMembershipManager.DEBUG_JAVAGROUPS = setting;
    ...
    try {
      ...
      if (async1 != null) {
        ...
        if (async2 != null) {
          ...
          // note: locator2 will usually fail to start as well because it
...
//              new LocalLogWriter(LogWriterImpl.ALL_LEVEL, System.out);
      ...
      assertTrue("Distributed system should not have disconnected",
          vm1.invokeBoolean(LocatorTest.class, "isSystemConnected"));
...
//              new LocalLogWriter(LogWriterImpl.ALL_LEVEL, System.out);
  ...
  public void testLeadFailureAndCoordShutdown() throws Exception {
    ...
    try {
      ...
      assertTrue("Distributed system should not have disconnected",
          vm2.invokeBoolean(LocatorTest.class, "isSystemConnected"));
      
      assertTrue("Distributed system should not have disconnected",
          locvm.invokeBoolean(LocatorTest.class, "isSystemConnected"));
      ...
      assertEquals("This test sometimes fails.  If the log contains " +
      		...
      		mem2, vm2.invoke(LocatorTest.class, "getLeadMember", new Object[]{}));
...
//            c.unregisterTestHook();
  ...
  public void testOneLocator() throws Exception {
    ...
    config.setLogFile(getUniqueName() + "-admin.log");
    ...
    assertTrue(admin.waitToBeConnected(2 * 1000));
    assertEquals(3, admin.getSystemMemberApplications().length);
  ...
  public void testMultipleLocators() throws Exception {
    ...
    try {
      ...
      try {
        ...
        final AdminDistributedSystem admin =
          AdminDistributedSystemFactory.getDistributedSystem(config);
        admin.connect();
...
//            props.setProperty("log-level", "fine");
    ...
    try {
      ...
      assertTrue(admin.waitToBeConnected(WAIT1_MS));
