...
public class ClientAuthorizationTestBase extends DistributedTestCase {
  ...
  public static Integer createCacheServer(Integer mcastPort, Object authProps,
      Object javaProps) {
  ...
  public static void createCacheServer(Integer mcastPort, Integer serverPort,
  ...
  protected List getAllGeneratorCombos() {
    ...
    while (authzCodeIter.hasNext()) {
      ...
      CredentialGenerator.ClassCode authClassCode;
      Iterator codeIter = codeGenList.iterator();
      while (codeIter.hasNext()) {
        authClassCode = (CredentialGenerator.ClassCode)codeIter.next();
        AuthzCredentialGenerator authzGen = AuthzCredentialGenerator
            .create(authzClassCode);
        if (authzGen != null) {
          CredentialGenerator cGen = CredentialGenerator.create(authClassCode);
          if (cGen != null) {
            cGen.init();
          ...
          if (opBlock.size() > 0) {
            ...
            server1.invoke(ClientAuthorizationTestBase.class,
                "createCacheServer", new Object[] {
                    SecurityTestUtil.getMcastPort(), port1, serverProps,
                    javaProps });
            ...
            if (!currentOp.equals(OperationWithAction.OPBLOCK_NO_FAILOVER)) {
              ...
              server2.invoke(ClientAuthorizationTestBase.class,
                  "createCacheServer", new Object[] {
                      SecurityTestUtil.getMcastPort(), port2, serverProps,
                      javaProps });
