...
public class SecurityTestUtil extends DistributedTestCase {
  ...
  public static void initDynamicRegionFactory() {
    ...
    DynamicRegionFactory.get().open(
        new DynamicRegionFactory.Config(null, null, false));
  }
  ...
  public static Integer createCacheServer(Properties authProps,
    ...
    if (locatorString != null && locatorString.length() > 0) {
      ...
      if (dsPort != null) {
        authProps.setProperty(DistributionConfig.START_LOCATOR_NAME,
            DistributedTestCase.getIPLiteral() + "[" + dsPort.toString() + ']');
      }
    }
    else {
      authProps.setProperty(DistributionConfig.MCAST_PORT_NAME, dsPort
          .toString());
    ...
    try {
      ...
      fail("Got unexpected exception when starting BridgeServer", ex);
    }
  ...
  public static void createCacheClient(String authInitModule,
    ...
    try {
      ...
      if (multiUserAuthMode) {
        ...
        // BridgeTestCase.configureConnectionPoolWithNameAndFactory below?
        // Actually setting it to false causes MultiuserAPIDUnitTest to fail.
        //poolFactory.setSubscriptionEnabled(false);
      }
      pool = BridgeTestCase.configureConnectionPoolWithNameAndFactory(factory,
...
//    }
  ...
  public static void startLocator(String name, Integer port, Object extraProps,
    ...
    try {
      ...
      LogWriter logger = new PureLogWriter(LogWriterImpl.CONFIG_LEVEL,
          logStream);
  ...
  public static List getAllGenerators() {
    ...
    while (codeIter.hasNext()) {
      CredentialGenerator gen = CredentialGenerator
          .create((CredentialGenerator.ClassCode)codeIter.next());
      if (gen != null) {
        gen.init();
        generators.add(gen);
      }
  ...
  public static void doMultiUserGetAll(Integer numOfUsers, Integer[] expectedResults) {
    ...
    for (int i = 0; i < numOfUsers; i++) {
      getLogWriter().info("GET_ALL: MultiUser# " + i);
      doGetAllP(Integer.valueOf(i), expectedResults[i]);
    }
  }
