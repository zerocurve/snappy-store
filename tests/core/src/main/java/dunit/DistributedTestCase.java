...
public abstract class DistributedTestCase extends TestCase
  implements java.io.Serializable {
  
  static {
    System.setProperty(InternalDistributedSystem.ENABLE_CREATION_STACK_PROPERTY, "true");
  }
  ...
  public static final boolean logPerTest = Boolean
      .getBoolean("dunitLogPerTest");
  ...
  public static void dumpStack() {
    com.gemstone.gemfire.internal.OSProcess.printStacks(0, Log.getLogWriter(), false);
  }
  ...
  protected static LogWriter createLogWriter(Properties config) {
    ...
    LogWriter logger = InternalDistributedSystem.createLogWriter(
        false/*appendToFile*/, false/*isLoner*/, false/*isSecurityLog*/, 
        dc, false, fos);
    ...
    nonDefault.put(DistributionConfig.LOG_OUTPUTSTREAM_NAME, loggerFileStream);
...
//    }
  ...
  /**
   * Tears down the test. This method is called by the final
   * {@link #tearDown()} method, and should be over ridden in its place.
   */
...
//          );
  ...
  public static void dumpMyThreads(LogWriter logger) {
    OSProcess.printStacks(0, logger, false);
  }
  
  protected static void setDiskStoreForGateway(Cache cache, String gatewayId, GatewayQueueAttributes queueAttributes)
  {
    // user is responsible to specify different diskstore or overflow dir
    // for gateways running on the same host or file system
    ...
    public void remove() {
      SerializableRunnable removeRunnable = new SerializableRunnable(
        ...
        public void run() {
          ...
          } catch (HydraRuntimeException noHydraLogger) {
          }

          LogWriter bgexecLogger = new LocalLogWriter(LogWriterImpl.ALL_LEVEL,
              System.out);
   ...
   * expected string is used by the Perl grepLog.pl utility and so can contain
  ...
  public static void incBBFlag(String name) {
    ...
    int currentVal = 0;
    final Object result = bb.getSharedMap().get(name);
    if (result != null && result instanceof Integer) {
      currentVal = ((Integer)result).intValue();
    }
    bb.getSharedMap().put(name, Integer.valueOf(++currentVal));
  ...
  public static void checkBBFlag(String name, int expectedValue)
    ...
    final Object result = bb.getSharedMap().get(name);
    if (result != null && result instanceof Integer) {
      gotValue = ((Integer)result).intValue();
    }
    bb.getSharedLock().unlock();
