/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.am.DatabaseMetaData
...
// GemStone changes BEGIN
    // switch to ODBC procedures when invoked from ODBC driver
    // using system property
    public final static String DATATYPE = Boolean.getBoolean(com.pivotal.gemfirexd.jdbc
        .ClientAttribute.USING_ODBC_DRIVER) ? "DATATYPE='ODBC'" : "DATATYPE='JDBC'";
// GemStone changes END
     ...
     * @see com.pivotal.gemfirexd.internal.impl.sql.catalog.DataDictionaryImpl#create_10_2_system_procedures(TransactionController,HashSet, UUID)
     * @see com.pivotal.gemfirexd.internal.impl.jdbc.EmbedDatabaseMetaData#getFunctions(String,String,String)
     */
...
// GemStone changes BEGIN
        cs.setStringX(5, DATATYPE + ";GETTABLETYPES=1; CURSORHOLD=" + cursorHold);
        /* (original code)
...
// GemStone changes BEGIN
        if (connection_.holdability() == ResultSet.HOLD_CURSORS_OVER_COMMIT) {
            cs.setStringX(7, DATATYPE + ";IMPORTEDKEY=1; CURSORHOLD=1");
        } else {
            cs.setStringX(7, DATATYPE + ";IMPORTEDKEY=1; CURSORHOLD=0");
        }
        /* (original code)
        if (connection_.holdability() == ResultSet.HOLD_CURSORS_OVER_COMMIT) {
...
// GemStone changes BEGIN
        if (connection_.holdability() == ResultSet.HOLD_CURSORS_OVER_COMMIT) {
            cs.setStringX(7, DATATYPE + ";EXPORTEDKEY=1; CURSORHOLD=1");
        } else {
            cs.setStringX(7, DATATYPE + ";EXPORTEDKEY=1; CURSORHOLD=0");
        }
        /* (original code)
        if (connection_.holdability() == ResultSet.HOLD_CURSORS_OVER_COMMIT) {
...
// GemStone changes BEGIN
    ...
    //
    //
    public java.sql.ResultSet getTypeInfo(short dataType) throws SQLException {
        try {
          synchronized (connection_) {
            if (agent_.loggingEnabled()) {
              agent_.logWriter_.traceEntry(this, "getTypeInfo(short)");
            }
            return getTypeInfoX(dataType);
          }
        } catch (SqlException se) {
          throw se.getSQLException(agent_ /* GemStoneAddition */);
        }
    ...
    private ResultSet getTypeInfoX(short dataType) throws SqlException {
        checkForClosedConnectionX();
...
// GemStone changes BEGIN
        return DATATYPE + ";DYNAMIC=0;REPORTPUBLICPRIVILEGES=1;CURSORHOLD=" + cursorHold;
        /* (original code)
...
// GemStone changes END
    ...
    private boolean getMetaDataInfoBoolean(int infoCallIndex) throws SQLException {
        try
        {
			if (metaDataInfoIsCached_) {
				return ((Short) metaDataInfoCache_[infoCallIndex]).shortValue() != 0;
			}
			metaDataInfoCall();
			return ((Short) metaDataInfoCache_[infoCallIndex]).shortValue() != 0;
        }
    ...
    // engine/com/pivotal/gemfirexd/internal/impl/sql/catalog/metadata_net.properties.
    // 
...
// GemStone changes BEGIN
                // changed to use Integer.parseInt()
                if (Integer.parseInt(st.nextToken(), 10) == type) {
                /* (original code)
...
// GemStone changes BEGIN
                // changed to use Integer.parseInt()
                if (Integer.parseInt(stForConc.nextToken(), 10) == type) {
                    while (stForConc.hasMoreTokens()) {
                        if (Integer.parseInt(stForConc.nextToken(), 10) == concurrency) {
                /* (original code)
...
// GemStone changes BEGIN
                // changed to use Integer.parseInt()
                if (Integer.parseInt(stForType.nextToken(), 10) == fromType) {
                    while (st.hasMoreTokens()) {
                        if (Integer.parseInt(st.nextToken(), 10) == toType) {
                /* (original code)
...
// GemStone changes END
    ...
    private PreparedStatement prepareMetaDataQuery(String cmd) throws SqlException {
        ...
        ps = (com.pivotal.gemfirexd.internal.client.am.PreparedStatement)
