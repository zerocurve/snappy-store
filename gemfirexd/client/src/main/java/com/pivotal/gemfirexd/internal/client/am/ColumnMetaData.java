/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.am.ColumnMetaData
...
// GemStone changes BEGIN
    public transient final Agent agent_;
// GemStone changes END
    ...
    transient private TObjectIntHashMap columnNameToIndexCache_ = null;
    ...
    void markClosed() {
      if (SanityManager.TraceSingleHop) {
        SanityManager.DEBUG_PRINT(SanityManager.TRACE_SINGLE_HOP,
            "Column MetaData marking this: " + this + " as closed: "/*, new Throwable()*/);
      }
    ...
    void checkForClosedStatement() throws SqlException {
        ...
        if (statementClosed_) {
          if (SanityManager.TraceSingleHop) {
            SanityManager.DEBUG_PRINT(SanityManager.TRACE_SINGLE_HOP,
                "checkForClosedStatement: " + this + " is closed: "/*, new Throwable()*/);
          }
...
// GemStone changes BEGIN
    public ColumnMetaData(final Agent agent) {
        agent_ = agent;
        logWriter_ = agent.logWriter_;
    /* (original code)
...
// GemStone changes BEGIN
    public ColumnMetaData(final Agent agent, int upperBound) {
        this(agent);
    /* (original code)
...
// GemStone changes BEGIN
    public void copyTo(ColumnMetaData target) {
      target.columns_ = this.columns_;
      target.nullable_ = this.nullable_;
      target.singleMixedByteOrDouble_ = this.singleMixedByteOrDouble_;
      target.sqldHold_ = this.sqldHold_;
      target.sqldReturn_ = this.sqldReturn_;
      target.sqldScroll_ = this.sqldScroll_;
      target.sqldSensitive_ = this.sqldSensitive_;
      target.sqldFcode_ = this.sqldFcode_;
      target.sqldKeytype_ = this.sqldKeytype_;
      target.sqldRdbnam_ = this.sqldRdbnam_;
      target.sqldSchema_ = this.sqldSchema_;
      target.sqlPrecision_ = this.sqlPrecision_;
      target.sqlScale_ = this.sqlScale_;
      target.sqlLength_ = this.sqlLength_;
      target.sqlType_ = this.sqlType_;
      target.sqlCcsid_ = this.sqlCcsid_;
      target.sqlName_ = this.sqlName_;
      target.sqlLabel_ = this.sqlLabel_;
      target.sqlUnnamed_ = this.sqlUnnamed_;
      target.sqlComment_ = this.sqlComment_;
      target.sqlUDTname_ = this.sqlUDTname_;
      target.sqlUDTclassName_ = this.sqlUDTclassName_;
      target.sqlxKeymem_ = this.sqlxKeymem_;
      target.sqlxGenerated_ = this.sqlxGenerated_;
      target.sqlxParmmode_ = this.sqlxParmmode_;
      target.sqlxCorname_ = this.sqlxCorname_;
      target.sqlxName_ = this.sqlxName_;
      target.sqlxBasename_ = this.sqlxBasename_;
      target.sqlxUpdatable_ = this.sqlxUpdatable_;
      target.sqlxSchema_ = this.sqlxSchema_;
      target.sqlxRdbnam_ = this.sqlxRdbnam_;
      target.protocolTypesCache_ = this.protocolTypesCache_;
      target.protocolTypeToOverrideLidMapping_ = this.protocolTypeToOverrideLidMapping_;
      target.mddOverrideArray_ = this.mddOverrideArray_;
      target.types_ = this.types_;
      target.clientParamtertype_ = this.clientParamtertype_;
      target.resultSetConcurrency_ = this.resultSetConcurrency_;
      target.columnNameToIndexCache_ = this.columnNameToIndexCache_;
    }
// GemStone changes END
    ...
    public String getColumnLabel(int column) throws SQLException {
        ...
        {
// GemStone changes BEGIN
            return getColumnLabelX(column);
        } catch (SqlException e) {
          throw e.getSQLException(agent_);
        }
    }

    public String getColumnLabelX(int column) throws SqlException {
// GemStone changes END
            checkForClosedStatement();
            checkForValidColumnIndex(column);
...
// GemStone changes END
    ...
    public boolean hasLobColumns() {
        for (int i = 0; i < columns_; i++) {
            switch (com.pivotal.gemfirexd.internal.client.am.Utils.getNonNullableSqlType(sqlType_[i])) {
            case DRDAConstants.DB2_SQLTYPE_BLOB:
            case DRDAConstants.DB2_SQLTYPE_CLOB:
                return true;
            default:
...
// GemStone changes BEGIN
            columnNameToIndexCache_ = new TObjectIntHashMap(this.columns_);
            String colName;
            for (int col = 0; col < this.columns_; col++) {
              if (this.sqlName_ != null &&
                      (colName = this.sqlName_[col]) != null) {
                this.columnNameToIndexCache_.putIfAbsent(colName, col + 1, 0);
                String tableName, schemaName;
                if (sqlxBasename_ != null
                    && (tableName = sqlxBasename_[col]) != null
                    && tableName.length() > 0) {
                  this.columnNameToIndexCache_.putIfAbsent(
                      tableName + '.' + colName, col + 1, 0);
                  if (sqlxSchema_ != null
                      && (schemaName = sqlxSchema_[col]) != null
                      && schemaName.length() > 0) {
                    this.columnNameToIndexCache_.putIfAbsent(schemaName + '.'
                        + tableName + '.' + colName, col + 1, 0);
                  }
                }
              }
        ...
        int columnPos = columnNameToIndexCache_.get(columnName);
        if (columnPos > 0) {
            return columnPos;
        }
        // search is case-insensitive as per JDBC spec
        columnPos = columnNameToIndexCache_.get(columnName
...
// GemStone changes END
    ...
    void assignColumnName(int column) {
        if (columnNameToIndexCache_ == null) {
            columnNameToIndexCache_ = new TObjectIntHashMap();
        }
// GemStone changes BEGIN
...
// GemStone changes BEGIN
   ...
   */
  public final short primaryKey(int column) throws SQLException {
    try {
      checkForClosedStatement();
      checkForValidColumnIndex(column);
      return sqlxKeymem_[column - 1];
    } catch (SqlException e) {
      throw e.getSQLException(agent_ /* GemStoneAddition */);
    }
  }
