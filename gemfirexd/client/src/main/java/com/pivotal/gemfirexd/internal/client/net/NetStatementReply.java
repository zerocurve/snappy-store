/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.net.NetStatementReply
...
public class NetStatementReply extends NetPackageReply implements StatementReplyInterface {
    ...
    public void readStatementID(StatementCallbackInterface statement) throws DisconnectException {
      startSameIdChainParse();
      parsePRPSQLStatementID(statement);
//      endOfSameIdChainData();
    }
    ...
    private void parsePRPSQLStatementID(StatementCallbackInterface statement) throws DisconnectException {
      int peekCP = peekCodePoint();
      if (peekCP == CodePoint.SQLCARD) {
        NetSqlca netSqlca = parseSQLCARD(null);
        int sqlcode = statement.completeSqlca(netSqlca);
        statement.completeStatementIDRead(null);
      } else if (peekCP == CodePoint.GETSTMTID) {
        parseLengthAndMatchCodePoint(CodePoint.GETSTMTID);
        int length = readInt();
        //        String statementUUID = readString();
        if (length != 0) {
          byte[] buf = readBytes(length);
          String statementUUID = new String(buf);
          statement.completeStatementIDRead(statementUUID);
        }
      } else {
        parseDescribeError(statement);
      }
    }
        ...
        throws DisconnectException
    {
        ...
        if (statement.cachedCursor_ != null) {
            ...
            try {
                rs = (NetResultSet)ClientDRDADriver.getFactory().newNetResultSet
                        (netAgent_,
   ...
   // Gemstone changes END
    ...
    //   SQLNUMROW; ROW LID 0x68; ELEMENT TAKEN 0(all); REP FACTOR 1
    //   SQLDAROW; ROW LID 0x60; ELEMENT TAKEN 0(all); REP FACTOR 0(all)
    NetSqlca parseSQLDARDarray(ColumnMetaData columnMetaData, StatementCallbackInterface statement,
                               boolean skipBytes) throws DisconnectException {
        int ddmLength = 0;
        ...
        // Gemstone changes BEGIN
        Section section = statement != null ? statement.getSection() : null;
        boolean singleHop = statement != null && section != null
            && section.isSingleHopInfoRequired();
        byte[] singleHopInfoBytes = null;
        SingleHopInformation singleHopInfoObj = null;
        if (singleHop) {
          int length = readInt();
          singleHopInfoBytes = readBytes(length);
          ByteArrayInputStream bais = new ByteArrayInputStream( singleHopInfoBytes );
          ObjectInputStream ois;
          try {
            ois = new ObjectInputStream( bais );
            singleHopInfoObj = (SingleHopInformation)ois.readObject();
            //System.out.println("KN: read the following single hop info object: " + singleHopInfoObj);
            if (SanityManager.TraceSingleHop) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_SINGLE_HOP,
                  "NetStatementReply::parseSQLDARDarray read the following single hop info object: " + singleHopInfoObj);
            }
            section.setSingleHopInformation(singleHopInfoObj);
          } catch (Exception e) {
            if (SanityManager.TraceSingleHop | SanityManager.TraceClientHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_SINGLE_HOP,
                  "NetStatementReply::parseSQLDARDarray got exception: " + e.getMessage(), e);
            }
            throw new DisconnectException(agent_, new ClientMessageId(
                SQLState.JAVA_EXCEPTION), e.getClass().getName(), e.getMessage());
          }
