/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.drda.CodePoint
...
// GemStone changes BEGIN
	...
	// Connection properties -- BEGIN

	// These flags clear the bits rather than setting since TRUE
	// has all the bits set in the higher nibble

	// Indicates a connection after failover.
	static final byte FAILOVER = (byte)0x1f;

	// Indicates that transactions should sync all commits and wait
	// for 2nd phase commit to finish on all nodes before returning.
	static final byte SYNC_COMMITS = (byte)0x2f;

	// Indicates that transactions should disable batching and flush
	// all operations immediately to all nodes.
	static final byte DISABLE_TX_BATCHING = (byte)0x4f;
        
        // Indicates that server can query HDFS data
        static final byte QUERY_HDFS = (byte)0x7F;

        // End flags that clear the bits in higher nibble

        // Indicates no streaming for connection.
        static final byte DISABLE_STREAMING = (byte)0x02;

        // Clear the DISABLE_STREAMING flag from input byte.
        static final byte CLEAR_DISABLE_STREAMING = ~(byte)0x02;

        // Indicates that callback invocations (listeners, DBSynchronizers etc)
        // should be skipped on the server for this connection
        static final byte SKIP_LISTENERS = (byte)0x04;

        // Clear the SKIP_LISTENER flag from input byte.
        static final byte CLEAR_SKIP_LISTENER = ~(byte)0x04;        

        // Indicates that constraint checks should be disabled for DMLs
        // performed from this connection.
        static final byte SKIP_CONSTRAINTS = (byte)0x08;

        // Clear the SKIP_CONSTRAINTS flag from input byte.
        static final byte CLEAR_SKIP_CONSTRAINTS = ~(byte)0x08;

        // These are additional properties that go as part of integer
        // in PRDDTA
        static final int SKIP_LOCKS = 0x1;

        // Connection properties -- END

        // Indicates that a BATCH statement output (int[]) is expected.
        static final byte OUTEXP_BATCH_OUTPUT = (byte)0x02;

        // Clear the OUTEXP_BATCH_OUTPUT flag from input byte.
        static final byte CLEAR_OUTEXP_BATCH_OUTPUT = ~(byte)0x02;

        // Combined with RTNSQLDA boolean, indicates that an unprepared
        /// statement has been executed from the client.
        static final byte RTNSQLDA_UNPREPARED = (byte)0x04;

        static final byte CLEAR_RTNSQLDA_UNPREPARED = ~(byte)0x04;
...
// GemStone changes BEGIN
	final static int RDBCMMMOD = 0x201E;
// GemStone changes END
...
// GemStone changes BEGIN
	...
	static final int DECDELPRD = 0x243C;

	// Decimal delimiter is a comma (,)
	static final int DECDELCMA = 0x243D;

	// Package Default
	static final int DFTPKG = 0x241E;

	// String delimiter is an apostrophe (')
	static final int STRDELAP = 0x2426;

	// String delimiter is a double quote (")
	static final int STRDELDQ = 0x2427;
// GemStone changes END
	...
	// Gemstone changes BEGIN
	static final int PKGNAMCSN_SHOP = 0x3000;
	    
	static final int PKGNAMCSN_SHOP_WITH_STR = 0x3001;
	
	static final int GETSTMTID = 0x3002;
	
	// ncj batch size
	static final int NCJBATCHSIZE = 0x3003;
	    
	// ncj cache size
	static final int NCJCACHESIZE = 0x3004;
	// Gemstone changes END
	...
	// Gemstone changes BEGIN
	// TYPSQLDA - Extended Output SQLDA
        static final int TYPSQLDA_X_OUTPUT_AND_SINGLEHOP_INFO = 6;
	// Gemstone changes END
...
// GemStone changes BEGIN
    // Current TXID generated on the server side
    static final int PBSD_TXID = 0xC003;
    
    static final int PBSD_SVRVER = 0xC004;
    
    static final int PBSD_REGIONS = 0xC005;
// GemStone changes END
...
// GemStone changes BEGIN
	// System/390 platform
	static final String TYPDEFNAM_QTDSQL370 = "QTDSQL370";
	// AS/400 platform
	static final String TYPDEFNAM_QTDSQL400 = "QTDSQL400";
// GemStone changes END
