/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.load.ColumnInfo
...
class ColumnInfo {
		...
		throws SQLException 
	{
		...
		//if insertColumnList is passed as parameter to the procedure, use that, otherwise use the one read from data file
...
// GemStone changes BEGIN
  private String readColumnListFromFile(String inputFileName,
...
// GemStone changes BEGIN
				jdbcColumnTypes.add(Integer.valueOf(dataType));
...
// GemStone changes BEGIN
            // changed to use Integer.parseInt()
            int colIndex = Integer.parseInt(colTypeInfo.substring(6,
                colTypeOffset), 10);
            int colType = Integer.parseInt(colTypeInfo.substring(
                colTypeOffset + 1), 10);
            /* (original code)
