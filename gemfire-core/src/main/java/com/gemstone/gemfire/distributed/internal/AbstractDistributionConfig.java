  ...
  implements DistributionConfig
{
    ...
    if (value != null && value.length() > 0 &&
      ...
      throw new IllegalArgumentException(
        LocalizedStrings.AbstractDistributionConfig_BIND_ADDRESS_0_INVALID_MUST_BE_IN_1
          .toLocalizedString(new Object[]{value, SocketCreator.getMyAddresses(true)
          }));
    }
  }
  ...
  protected String checkLocators(String value) {
    ...
    while (st.hasMoreTokens()) {
      ...
      StringBuilder locatorsb = new StringBuilder();  // string for this locator is accumulated in this buffer
  ...
  protected void checkAttributeName(String attName) {
    if (!attName.startsWith(SECURITY_PREFIX_NAME)
        && !attName.startsWith(GFXD_USERDEFINED_PREFIX_NAME)
  ...
  static {
    String[] myAtts = new String[] {
      ...
      LOG_LEVEL_NAME, LOCATORS_NAME,REMOTE_LOCATORS_NAME,
...
//      DURABLE_CLIENT_KEEP_ALIVE_NAME,
  ...
  static {
   // skip this for hadoop loner mode
   if (InternalDistributedSystem.isHadoopGfxdLonerMode()) {
     dcAttDescriptions = Collections.emptyMap();
   }
   else {
