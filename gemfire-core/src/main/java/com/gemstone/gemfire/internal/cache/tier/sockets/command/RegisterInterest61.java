...
//  region data policy
    ...
    if (logger.fineEnabled()) {
      logger.fine(servConn.getName() + ": Received register interest 61 request ("
    ...
    if (key == null || regionName == null) {
      ...
      if (logger.warningEnabled()) {
        logger.warning(LocalizedStrings.TWO_ARG_COLON,
          new Object[] {servConn.getName(), message.toLocalizedString()});
      }
      writeChunkedErrorResponse(msg, MessageType.REGISTER_INTEREST_DATA_ERROR,
      ...
      if (!isPrimary) {
        ...
        if (logger.fineEnabled()) {
          ...
          logger.fine(servConn.getName()
              + ": Sent register interest response for region " + regionName
              + " key " + key);
        }
