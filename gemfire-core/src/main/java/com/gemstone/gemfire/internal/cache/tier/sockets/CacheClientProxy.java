...
//
   ...
   */
  protected static final int MAXIMUM_SHUTDOWN_PEEKS = 50;
...
//   {
//     // this._logger.warning("Is input port " + clientPort + " contained in " +
//     // this._logger.warning("Does input host " + clientHost + " equal " +
//     // this._remoteHostAddress+ ": " + representsClientHost(clientHost));
//     // this._logger.warning("representsClientVM: " +
...
//   }
  ...
  /**
   * Returns this proxy's <code>LogWriterI18n</code>.
   * @return this proxy's <code>LogWriterI18n</code>
   */
  ...
  public boolean closeClientCq(String clientCQName) throws CqException {
    ...
    try {
      ...
      if (drainLocked) {
        ...
        _logger.info(LocalizedStrings.ONE_ARG, msg);
      ...
      if (isPaused() && !isConnected()) {
        ...
        if (cqService != null) {
          CqQuery cqToClose = cqService.getCq(cqService.constructServerCqName(
  ...
  protected void pauseDispatching() {
    ...
    if (_logger.fineEnabled()) {
      _logger.fine(this + ": Pausing processing");
    }
  ...
  protected void terminateDispatching(boolean checkQueue) {
    ...
    try {
    if (_logger.fineEnabled()) {
      _logger.fine(this + ": Terminating processing");
    }
    if (this._messageDispatcher == Thread.currentThread()) {
    ...
    if (this.testAndSetPaused(false)) {
      if (_logger.fineEnabled()) {
        _logger.fine(this + ": Paused but terminating processing");
      }
    ...
    try {
      ...
      if (this._messageDispatcher.isAlive()) {
        if (this._socket != null && !this._socket.isClosed()) {
          SocketCreator.asyncClose(this._socket, this.getLoggerI18n(), this._remoteHostAddress, null);
  ...
  private void closeNonDurableCqs(){
    ...
    if (cqService != null) {
      try {
        ...
        if (_logger.warningEnabled()) {
          _logger.warning(LocalizedStrings.CacheClientProxy_CQEXCEPTION_WHILE_CLOSING_NON_DURABLE_CQS_0,
...
//      throw e;
        ...
        if (policy == InterestResultPolicy.KEYS_VALUES) {
          ...
          if (entry.value instanceof byte[]) {
            ...
            try {
              ...
              this._logger.warning(LocalizedStrings.CacheClientProxy_THE_FOLLOWING_EXCEPTION_OCCURRED_0, entry.value, e);
            }
          }
          VersionTag tag = entry.versionTag;
          ClientProxyMembershipID serverProxyId = ClientProxyMembershipID
              .getNewProxyMembership(this._cache.getDistributedSystem());
          ClientUpdateMessage updateMessage = new ClientUpdateMessageImpl(
              EnumListenerEvent.AFTER_CREATE, lr, keyOfInterest, value, null,
              (isObject ? (byte) 0x01 : (byte) 0x00), null, serverProxyId,
              new EventID(this._cache.getDistributedSystem()), this._logger, tag);
          CacheClientNotifier.routeSingleClientMessage(updateMessage, serverProxyId);
        }
  ...
  private void notifySecondariesAndClient(String regionName,
    ...
    ClientInterestMessageImpl message = new ClientInterestMessageImpl(
        new EventID(this._cache.getDistributedSystem()), this._logger,
        regionName, keyOfInterest, interestType, policy.getOrdinal(),
        isDurable, !receiveValues, ClientInterestMessageImpl.REGISTER);
    ...
    if (Version.GFE_701.compareTo(this.clientVersion) > 0
      ...
      for (Iterator i = ((List) keyOfInterest).iterator(); i.hasNext();) {
        this._messageDispatcher.enqueueMessage(new ClientInterestMessageImpl(
            new EventID(this._cache.getDistributedSystem()), this._logger,
            regionName, i.next(), interestType, policy.getOrdinal(), isDurable,
  ...
  private void notifySecondariesAndClient(String regionName,
    ...
    ClientInterestMessageImpl message = new ClientInterestMessageImpl(
        new EventID(this._cache.getDistributedSystem()), this._logger,
        regionName, keyOfInterest, interestType, (byte) 0, isDurable,
        !receiveValues, ClientInterestMessageImpl.UNREGISTER);
    notifySecondariesOfInterestChange(message);
    ...
    if (Version.GFE_701.compareTo(this.clientVersion) > 0
      ...
      for (Iterator i = ((List) keyOfInterest).iterator(); i.hasNext();) {
        this._messageDispatcher.enqueueMessage(new ClientInterestMessageImpl(
            new EventID(this._cache.getDistributedSystem()), this._logger,
            regionName, i.next(), interestType, (byte) 0, isDurable,
            !receiveValues, ClientInterestMessageImpl.UNREGISTER));
      }
    } else {
      this._messageDispatcher.enqueueMessage(message);
  ...
  protected void notifySecondariesOfInterestChange(ClientInterestMessageImpl message) {
    if (this._logger.fineEnabled()) {
      ...
      this._logger.fine(buffer.toString());
    }
  ...
  protected void addFilterRegisteredClients(String regionName,
    ...
    try {
      ...
      this._logger.warning(LocalizedStrings.CacheClientProxy_0_INTEREST_REG_FOR_0_FAILED, regionName + "->" + keyOfInterest, e);
    }
  }
  */
  ...
  public void flushForInterestRegistration(String regionName, DistributedMember target) {
    ...
    if (r == null) {
      if (this._logger.fineEnabled()) {
        this._logger.fine("Unable to find region '" + regionName + "' to flush for interest registration");
      }
    } else if (r.getAttributes().getScope().isDistributed()) {
      if (this._logger.fineEnabled()){
        this._logger.fine("Flushing region '" + regionName + "' for interest registration");
      }
  ...
  private boolean postDeliverAuthCheckPassed(ClientUpdateMessage clientMessage) {
    ...
    if (AcceptorImpl.isAuthenticationRequired()
      ...
      if (clientCq != null && !clientCq.isEmpty()) {
        if (_logger.fineEnabled()) {
          _logger.fine("CCP clientCq size before processing auth "
              + clientCq.size());
        }
        LogWriterI18n logger = this._cache.getSecurityLoggerI18n();
        ...
        if (_logger.fineEnabled()) {
          _logger.fine("CCP clientCq names array size " + cqNames.length);
        }
        for (int i = 0; i < cqNames.length; i++) {
          try {
            if (_logger.fineEnabled()) {
              _logger.fine("CCP clientCq name " + cqNames[i]);
            }
            ...
            if (this.proxyID.isDurable() && this.getDurableKeepAlive()
              ...
              synchronized (this.clientUserAuthsLock) {
                ...
                if (_logger.fineEnabled() && postAuthCallback == null) {
                  _logger.fine("CCP clientCq post callback is null");
                }
          ...
          if (_logger.fineEnabled()) {
            _logger.fine("CCP clientCq size after processing auth "
                + clientCq.size());
          }
        }
        ...
        if (!clientMessage.hasCqs(this.proxyID)) {
          ...
          if (BridgeServerImpl.VERBOSE || _logger.fineEnabled()) {
            _logger.info(LocalizedStrings.DEBUG, this
                + ": Not adding message to queue"
                + ". It is not interested in this region and key: "
                + clientMessage);
          }
          return false;
        }
      ...
      if (logger.finestEnabled()){
        logger.finest(this.toString()
            + ": Invoking authorizeOperation for message: " + clientMessage);
      }
  ...
  protected void deliverMessage(Conflatable conflatable)
  {
    ...
    if (clientMessage.needsNoAuthorizationCheck() || postDeliverAuthCheckPassed(clientMessage)) {
      ...
      if (this.messageDispatcherInit) {
        synchronized (this.queuedEventsSync) {
          if (this.messageDispatcherInit) {  // Check to see value did not changed while getting the synchronize lock.
            if (this._logger.fineEnabled()) {
              this._logger.fine("Message dispatcher for proxy " + this + 
              " is getting initialized. Adding message to the queuedEvents.");
            }
      ...
      if (this._messageDispatcher != null) {
        ...
        if (this._logger.fineEnabled()) {
          this._logger.fine("Message is not added to the queue. " + 
              " Message dispatcher for proxy :" + this + " doesn't exist. ");
        }
      }
    }
  }
  ...
  private OperationContext getOperationContext(ClientMessage cmsg,
    ...
    if (cmsgimpl.isCreate()) {
      if (DynamicRegionFactory.regionIsDynamicRegionList(regionName)) {
        ...
        opctxt = new PutOperationContext(cmsgimpl.getKeyOfInterest(), cmsgimpl
      ...
      if (DynamicRegionFactory.regionIsDynamicRegionList(regionName)) {
        ...
        opctxt = new DestroyOperationContext(cmsgimpl.getKeyOfInterest(), true);
      }
    }
      ...
      opctxt = new InvalidateOperationContext(cmsgimpl.getKeyOfInterest(), true);
    }
    else if (cmsgimpl.isClearRegion()) {
      opctxt = new RegionClearOperationContext(true);
    }
    return opctxt;
  }
  ...
  public void initializeMessageDispatcher() throws CacheException
  {
    ...
    try {
      if (this._logger.fineEnabled()) {
        this._logger.fine(this.toString()
            + ": Initializing message dispatcher with capacity of "
            + _maximumMessageCount + " entries");
      }
      ...
      if (this._logger.fineEnabled()) {
        this._logger.fine(this.toString() + " draining " + this.queuedEvents.size() +
            " events from init queue into intialized queue");
      }
  ...
  protected void startOrResumeMessageDispatcher(boolean processedMarker) {
    ...
    if (this.isPrimary) {
      ...
      if (!processedMarker) {
        ...
        this._messageDispatcher.enqueueMarker(new ClientMarkerMessageImpl(eventId, this._logger));
      }
  ...
  static protected class ClientInterestList
   {
     ...
     * The GemFire log writer
     */
    final private LogWriterI18n _logger;
    ...
    {
      if (_logger.fineEnabled()) {
        _logger.fine(ccp + ": registerClientInterest region=" + regionName
            + " key=" + keyOfInterest);
      }
    ...
    {
      if (_logger.fineEnabled()) {
        _logger.fine(ccp + ": unregisterClientInterest region=" + regionName
            + " key=" + keyOfInterest);
      }
    ...
    private void handleInterestEvent(String regionName, Set keysOfInterest,
      ...
      if (NOTIFY_REGION_ON_INTEREST && this.ccp.isPrimary()
        ...
        try {
          ...
          this._logger.warning(LocalizedStrings.
              CacheClientProxy_REGION_NOTIFICATION_OF_INTEREST_FAILED, e);
        }
      }
...
//    private boolean _messageQueueCapacityReachedWarning = false;
    ...
    protected MessageDispatcher(CacheClientProxy proxy, String name) throws CacheException {
      super(LogWriterImpl.createThreadGroup(name, proxy.getLoggerI18n()), name);
      ...
      try {
        ...
        if (getLoggerI18n().fineEnabled() && (createDurableQueue || canHandleDelta)) {
      ...
      try {
        for (int numberOfPeeks = 0; numberOfPeeks < MAXIMUM_SHUTDOWN_PEEKS;
          ...
          try {
            ...
            if (getLoggerI18n().fineEnabled()) {
              getLoggerI18n().fine(this + ": Exception occurred while trying to stop dispatching",
                               e);
            }
          }
          finally {
    ...
    protected void drainClientCqEvents(ClientProxyMembershipID clientId, CqQuery cqToClose) {
...
//        SystemFailure.checkFailure(); DM's stopper does this
      ...
      try {
        ...
        if (getLoggerI18n().fineEnabled()) {
          getLoggerI18n()
              .fine(this
                  + ": After flagging the dispatcher to stop , the residual List of messages to be dispatched="
                  + list + " size ="+ list.size());
        }
        if (list.size() > 0) {
          long start = getStatistics().startTime();
        ...
        throws IOException
    {
     ...
     if (clientMessage instanceof ClientUpdateMessage) {
        ...
        if (getLoggerI18n().finerEnabled()) {
          ...
          getLoggerI18n().finer(msg.toString());
        }
     ...
     if (!this._proxy.isPaused()) {
      ...
      isDispatched = true;
       ...
       if (getLoggerI18n().fineEnabled()) {
         getLoggerI18n().fine("Message Dispatcher of a Paused CCProxy "
           + "is trying to dispatch message");
         }
     }
     if (isDispatched) {
    ...
    protected void enqueueMessage(Conflatable clientMessage)
    {
      try {
          ...
          if(this._proxy.isPaused() && this._proxy.isDurable()){
            ...
            if (getLoggerI18n().fineEnabled()) {
              getLoggerI18n().fine(this + ": Queued message while Durable Client is away " + clientMessage);
            }
          } else {
// [bruce] we don't really know that it was added, so don't log this
//            if (getLoggerI18n().fineEnabled() || BridgeServerImpl.VERBOSE) {
//              getLoggerI18n().info(LocalizedStrings.DEBUG, this + " added message to queue: " + clientMessage);
//          }
      }
      }
      catch (CancelException e) {
    ...
    protected void enqueueMarker(ClientMessage message) {
      try {
        if (getLoggerI18n().fineEnabled()) {
          getLoggerI18n().fine(this + ": Queueing marker message. <"
              + message + ">. The queue contains " + getQueueSize() + " entries.");
        }
        ...
        if (!isStopped()) {
          if (getLoggerI18n().severeEnabled()) {
            getLoggerI18n().severe(LocalizedStrings.CacheClientProxy_0__EXCEPTION_OCCURRED_WHILE_ATTEMPTING_TO_ADD_MESSAGE_TO_QUEUE, this, e);
          }
        }
      }
    }
