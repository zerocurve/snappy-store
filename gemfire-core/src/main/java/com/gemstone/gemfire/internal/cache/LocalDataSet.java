...
public final class LocalDataSet implements Region, QueryExecutor {
  ...
  final private transient TXStateInterface txState;
  ...
  public Set<Object> entrySet(boolean recursive) {
  ...
  public Set<Object> entries(boolean recursive) {
    return this.proxy.new PREntriesSet(getBucketSet(), this.txState);
  }

  public Collection values() {
    this.proxy.checkReadiness();
    return this.proxy.new ValuesSet(getBucketSet(), this.txState);
  }

  public Set keys() {
    return this.proxy.new KeysSet(getBucketSet(), this.txState);
  }
  ...
  public Entry getEntry(Object key) {
    if(isInDataSet(key, null)) {
      return this.proxy.getEntry(key, this.txState);
    } else {
      return null;
    }
  }
  ...
  public int size() {
    return this.proxy.entryCount(this.txState, getBucketSet());
  }
  ...
  public Object get(Object key, Object aCallbackArgument)
    ...
    if (isInDataSet(key, aCallbackArgument)) {
      if(this.txState == null){
        ...
        return this.proxy.get(key, aCallbackArgument, true, false, false, null,
        this.txState, null, null, false, true/*allowReadFromHDFS*/);
      }
    }
    else {
      return null;
          ...
          for (;;) { // Loop till we get valid value
            while (curBucketIter == null || !(hasNext = curBucketIter.hasNext())) { // Loop all the buckets.
              ...
              BucketRegion br = proxy.getDataStore().getLocalBucketById(
                curBucketId);
              if (br == null) {
                throw new BucketMovedException("Bucket region is moved or "
                    + "destroyed during iteration of local data set.",
                    this.curBucketId, proxy.getFullPath());
              }
              br.waitForData();
