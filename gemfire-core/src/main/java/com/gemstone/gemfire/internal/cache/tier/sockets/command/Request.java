...
//    valuePart = null;  (redundant assignment)
    ...
    if (logger.fineEnabled()) {
      logger.fine(servConn.getName() + ": Received get request ("
    ...
    if (key == null || regionName == null) {
      ...
      if (region == null) {
        ...
        try {
          getValueAndIsObject(region, key, callbackArg, logger, servConn,
              ((LocalRegion)region).discoverJTA(), valueAndIsObject);
        }
        catch (Exception e) {
          writeException(msg, e, false, servConn);
          servConn.setAsTrue(RESPONDED);
  ...
  public void getValueAndIsObject(Region region, Object key,
      Object callbackArg, LogWriterI18n logger,
      ServerConnection servConn, TXStateInterface tx, Object[] result) {
    ...
    Object data  = ((LocalRegion)region).get(key, callbackArg, true, true,
        true, id, tx, null, null, false, true/*allowReadFromHDFS*/);
