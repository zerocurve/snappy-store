...
public class Message  {
  ...
  protected int transactionId = TXManagerImpl.NO_TX;
  ...
  throws IOException {
    ...
    if (len < 0) {
      if (this.logger.configEnabled()) {
        this.logger.config(LocalizedStrings.Message_RPL_NEG_LEN__0, len);
      }
      throw new IOException(LocalizedStrings.Message_DEAD_CONNECTION.toLocalizedString());
    }    
...
//          res = 0;
  ...
  public void send(ServerConnection servConn, LogWriterI18n logger,
      int transactionId) throws IOException {
    this.sc = servConn;
    sendBytes(true);
  }

  public void send(LogWriterI18n logger, int transactionId)
    throws IOException {
    sendBytes(true);
  }
