  ...
  public void cmdExecute(Message msg, ServerConnection servConn, long start)
    ...
    if (logger.fineEnabled()) {
      logger.fine(servConn.getName()
          + ": Received unregister interest request (" + msg.getPayloadLength()
          + " bytes) from " + servConn.getSocketString() + " for region "
          + regionName + " key " + key);
    }
    ...
    if ((key == null) && (regionName == null)) {
      ...
      if (logger.fineEnabled()) {
        logger.fine(servConn.getName()
            + ": Sent unregister interest response for region " + regionName
            + " key " + key);
      }
