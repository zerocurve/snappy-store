...
public class DistributionLocator  {
  ...
  private static LocalLogWriter log = new LocalLogWriter(LogWriterImpl.CONFIG_LEVEL);
...
//          address = null; // was InetAddress.getLocalHost(); (redundant assignment)
        ...
        lockFile = ManagerInfo.setLocatorStarting(directory, log, port, address);
        ...
        log.severe(LocalizedStrings.DistributionLocator_COULD_NOT_BIND_LOCATOR_TO__0__1, new Object[] {address, Integer.valueOf(port)});
        System.exit(1);

      } catch (Exception ex) {
        log.severe(LocalizedStrings.DistributionLocator_COULD_NOT_START_LOCATOR, ex);
        System.exit(1);
      }
  }

  protected static String makeDaemon() {
    try {
