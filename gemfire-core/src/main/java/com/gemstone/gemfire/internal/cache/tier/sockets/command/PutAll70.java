...
//      isObjectMap.put(key, new Boolean(isObject));
    ...
    if (logger.fineEnabled()) {
      logger.fine(servConn.getName() + ": Sending putAll70 response back to "
          + servConn.getSocketString() + " for region " + regionName + ": " + response);
    }
    // Starting in 7.0.1 we do not send the keys back
    if (response != null && Version.GFE_70.compareTo(servConn.getClientVersion()) < 0) {
      if (logger.fineEnabled()) {
        logger.fine("setting putAll keys to null");
      }
