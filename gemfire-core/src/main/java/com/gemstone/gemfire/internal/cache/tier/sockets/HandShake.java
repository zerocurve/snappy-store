...
public class HandShake implements ClientHandShake
{
   ...
   * @since 5.7
   */
  public static short currentClientVersion = ConnectionProxy.VERSION.ordinal();
  ...
  //}
   ...
   public HandShake(HandShake handShake) {
     ...
     this.clientPublicKey = handShake.clientPublicKey;
     this.clientReadTimeout = handShake.clientReadTimeout;
     this.clientSKAlgo = handShake.clientSKAlgo;
  ...
  private void setOverrides() {
    ...
    // set via tests.
    if (HandShake.currentClientVersion >= Version.GFE_603.ordinal()) {
      byte override = 0;
      /*
  ...
  public byte write(DataOutputStream dos, DataInputStream dis, byte communicationMode, int replyCode,
    ...
    try {
      ...
      Version.writeOrdinal(hdos, currentClientVersion, true);
  ...
  public void writeCredentials(DataOutputStream dos, DataInputStream dis,
    ...
    try {
      LogWriterI18n securityLogger = this.system.getSecurityLogWriter().convertToLogWriterI18n();
      securityLogger.fine("HandShake: using Diffie-Hellman key exchange with algo " + dhSKAlgo);
      ...
      if (acceptanceCode == REPLY_OK) {
        ...
        if (requireAuthentication) {
          ...
          securityLogger.fine("HandShake: Successfully verified the "
        ...
        //  byte[] encBytes = encrypt.doFinal(hdos.toByteArray());
          byte[] encBytes = encryptBytes(hdos.toByteArray(), dhSKAlgo, this.clientPublicKey);
...
//          byte[] encBytes = encrypt.doFinal(hdos.toByteArray());
  ...
  public byte [] encryptBytes(byte[] data) throws Exception
  {
    if( this.appSecureMode == CREDENTIALS_DHENCRYPT) {
      ...
      return encryptBytes(data, algo, this.clientPublicKey);
    } else {
      return data;
    }
  }
...
//This assumes that authentication is the last piece of info in handshake
  public Properties readCredential(DataInputStream dis,
    ...
    try {
      ...
      if (secureMode == CREDENTIALS_NONE) {
        ...
        LogWriterI18n securityLogger = system.getSecurityLogWriter().convertToLogWriterI18n();
       ...
       // PublicKey pubKey = null;
        if (requireAuthentication) {
          ...
          if (sendAuthentication) {
            ...
            securityLogger.fine("HandShake: sent the signed client challenge");
          }
          else {
          ...
          securityLogger.fine("HandShake: sent the public key and challenge");
          dos.flush();

          // Read and decrypt the credentials
          byte[] encBytes = DataSerializer.readByteArray(dis);
          byte[] credentialBytes = decryptBytes(encBytes, this.clientSKAlgo, this.clientPublicKey);
  ...
  {  
    if( this.appSecureMode == CREDENTIALS_DHENCRYPT) {
      ...
      return decryptBytes(data, algo, this.clientPublicKey);
    } else {
      return data;
    }
  }
  ...
  //additional byte of wan site needs to send for Gateway BC
    if (communicationMode == Acceptor.GATEWAY_TO_GATEWAY) {
      Version.writeOrdinal(dos, ServerHandShakeProcessor.currentServerVersion,
          true);
    }
    ...
    if (communicationMode == Acceptor.GATEWAY_TO_GATEWAY
        ...
        && ServerHandShakeProcessor.currentServerVersion >= Version.GFE_66
            .ordinal()) {
  ...
  public void greetNotifier(Socket sock, boolean isPrimary, ServerLocation location) throws IOException, AuthenticationRequiredException,
    ...
    try {
      ...
      if (currentClientVersion < Version.GFE_61.ordinal()) {
        return;
      }
...
//    if (other == null) return false;
  ...
  private Properties getCredentials(DistributedMember member) {
    ...
    return getCredentials(authInitMethod, this.system.getSecurityProperties(),
        member, false, this.system.getLogWriter(), 
  ...
  public static Properties readCredentials(DataInputStream dis,
    ...
    try {
      ...
      }else if (secureMode == SECURITY_MULTIUSER_NOTIFICATIONCHANNEL)
      {
        ...
        system.getLogWriter().fine("readCredential where multiuser mode creating callback connection");
      }
    }
    catch (IOException ex) {
      throw ex;
  ...
  public Principal verifyCredentials() throws AuthenticationRequiredException,
    ...
    return verifyCredentials(methodName, this.credentials, this.system
        .getSecurityProperties(), this.system.getLogWriter(), this.system
