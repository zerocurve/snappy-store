...
//  private LogWriterI18n logger = InternalDistributedSystem.getLoggerI18n();
  ...
  public ObjectName registerGfxdMBean(Object object, ObjectName objectName, boolean isSqlFireMBean) {
    ObjectName newObjectName = objectName;
    try {
      if (!isSqlFireMBean) {
        String member = getMemberNameOrId(distMember);
        ...
        newObjectName = ObjectName.getInstance(OBJECTNAME__PREFIX_GFXD + objectKeyProperty +
            KEYVAL_SEPARATOR + "member=" + member);
      }
      mbeanServer.registerMBean(object, newObjectName);
      this.localGemFireMBean.put(newObjectName, object);
  ...
  /**
  *
  * This method will query the MBean from GemFire Domain
  *
  * @param objectNamePattern
  */
 public Set<ObjectName> queryMBeans(ObjectName objectNamePattern) {
   Set<ObjectName> queriedNames = Collections.emptySet();
   try {

     queriedNames = mbeanServer.queryNames(objectNamePattern, null);
   } catch (NullPointerException e) {
     throw new ManagementException(e);
   }
   return queriedNames;
  ...
  public static ObjectName getObjectName(String name) {
    try {
      ...
      throw new ManagementException("For MBean name string: " + name, e);
    } catch (NullPointerException e) {
      throw new ManagementException(e);
    }
  }
  ...
  public String getAlertLevel() {
    String alertLevel = DEFAULT_ALERT_LEVEL;
    //TODO Abhishek:: read from cluster MBean too
    DistributedSystemMXBean distributedSystemMXBean = getDistributedSystemMXBean();
    if (distributedSystemMXBean != null) {
      alertLevel = distributedSystemMXBean.getAlertLevel();
    }
    return alertLevel;
  }

  public DistributedMember getDistibutedMember() {
    return distMember;
  }
