...
public class CacheFactory {
  ...
  private static Cache basicGetInstance(DistributedSystem system, boolean closeOk) {
    ...
    if (initReq == LocalRegion.ANY_INIT
      ...
      //InternalDistributedSystem.getAnyInstance().getLogWriter().warning("KN: without sync");
      return basicGetInstancePart2(system, closeOk);
    } else {
      //InternalDistributedSystem.getAnyInstance().getLogWriter().warning("KN: with sync");
      synchronized (CacheFactory.class) {
        return basicGetInstancePart2(system, closeOk);
      }
    }
