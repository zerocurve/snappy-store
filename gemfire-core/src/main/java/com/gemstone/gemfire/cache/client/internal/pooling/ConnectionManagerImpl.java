...
//  private final long pingInterval; // ms // make this an int
  ...
  public ConnectionManagerImpl(String poolName,
                               ...
                               long pingInterval, CancelCriterion cancelCriterion,
                               PoolStats poolStats) {
...
//     logger.info("DEBUG: exchangeConnection internalDestroy(" + oldPC +")");
  ...
  private PooledConnection addConnection(Connection conn) {
    ...
    if(logger.fineEnabled()) {
      logger.fine("Created a new connection. " + pooledConn + " Connection count is now " + connectionCount);
    }
    return pooledConn;
  }
  ...
  private void destroyConnection(PooledConnection connection) {
    ...
    try {
      if (allConnectionsMap.removeConnection(connection)) {
        if(logger.fineEnabled()) {
          logger.fine("Invalidating connection " + connection + " connection count is now " + connectionCount);
        }
...
//         logger.info("DEBUG: destroyConnection conCount(-1)->" + connectionCount);
  ...
  protected void invalidateServer(Endpoint endpoint) {
    ...
    try {
      ...
      if(logger.fineEnabled()) {
        logger.fine("Invalidating " +
                    badConnections.size() + " connections to server " + endpoint);
      }
...
//       logger.info("DEBUG: invalidateServer conCount(" + (-badConnections.size()) + ")->" + connectionCount);
  ...
  public void returnConnection(Connection connection, boolean accessed) {
    ...
    if (shouldClose) {
      try {
        pooledConn.internalClose(false);
      } catch (Exception e) {
        logger.warning(
  ...
  public void close(boolean keepAlive) {
    if(logger.fineEnabled()) {
      logger.fine("Shutting down connection manager with keepAlive " + keepAlive);
    }
    ...
    try {
      if (this.loadConditioningProcessor != null) {
        ...
        if(!this.loadConditioningProcessor.awaitTermination(PoolImpl.SHUTDOWN_TIMEOUT, TimeUnit.MILLISECONDS)) {
          logger.warning(LocalizedStrings.ConnectionManagerImpl_TIMEOUT_WAITING_FOR_LOAD_CONDITIONING_TASKS_TO_COMPLETE);
        }
      }
    }
    catch (RuntimeException e) {
      logger.error(LocalizedStrings.ConnectionManagerImpl_ERROR_STOPPING_LOADCONDITIONINGPROCESSOR, e);
    }
    catch (InterruptedException e) {
      logger.error(LocalizedStrings.ConnectionManagerImpl_INTERRUPTED_STOPPING_LOADCONDITIONINGPROCESSOR, e);
    }
    ...
    if (createConnection) {
      ...
      try {
        ...
        if (logger.infoEnabled()) {
          logger.info(LocalizedStrings.ConnectionManagerImpl_UNABLE_TO_PREFILL_POOL_TO_MINIMUM_BECAUSE_0, ex, null);
        }
        return false;
      }
      finally {
...
//             logger.info("DEBUG: prefillConnection conCount(-1)->" + connectionCount);
            if(logger.fineEnabled()) {
              logger.fine("Unable to prefill pool to minimum, connection count is now " + connectionCount);
            }
          }
          else {
            ...
            freeConnection.signalAll();
            if(logger.fineEnabled()) {
              logger.fine("Prefilled connection  " + connection + " connection count is now " + connectionCount);
            }
          }
        }
        finally {
...
//         logger.info("DEBUG: lifetimeTask=" + this);
        ...
        logger.warning(LocalizedStrings.ConnectionManagerImpl_LOADCONDITIONINGTASK_0_ENCOUNTERED_EXCEPTION, this, t);
  ...
  protected class IdleExpireConnectionsTask implements Runnable {
    public void run() {
      try {
        ...
        SystemFailure.checkFailure();
        logger.warning(LocalizedStrings.ConnectionManagerImpl_IDLEEXPIRECONNECTIONSTASK_0_ENCOUNTERED_EXCEPTION, this, t);
        // Don't rethrow, it will just get eaten and kill the timer
      }
    }
  }
  ...
  protected class PrefillConnectionsTask extends PoolTask {
    ...
    @Override
    public void run2() {
      logger.finer("Prefill Connections task running");
    ...
    private synchronized void removeFromEndpointMap(Endpoint endpoint, PooledConnection connection) {
      ...
      if (endpointConnections != null) {
        ...
        if(endpointConnections.size() == 0) {
          map.remove(endpointConnections);
        }
      }
    }
...
//           logger.info("DEBUG: scheduling lifetimeTask=" + task);
    ...
    public void checkIdleExpiration() {
      ...
      synchronized (this) {
        ...
        logger.finer("Looking for connections to expire");
        ...
        for (Iterator it = this.allConnections.iterator();
          ...
          if (pc.shouldDestroy()) {
            ...
            long remainingIdle = pc.doIdleTimeout(now, idleTimeoutNanos, logger);
            if (remainingIdle >= 0) {
              if (remainingIdle == 0) {
...
//      if (toClose != null) (cannot be null) 
      {
        for (Iterator itr = toClose.iterator(); itr.hasNext(); ) {
          ...
          if (logger.fineEnabled()) {
            logger.fine("Idle connection detected. Expiring connection " + connection);
          }
          try {
            connection.internalClose(false);
          }
          catch (Exception e) {
            logger.warning(LocalizedStrings.ConnectionManagerImpl_ERROR_EXPIRING_CONNECTION_0, 
                connection);
          }
        }
      }
...
//                         + firstLife + " nanos");
  ...
  private void logInfo(StringId message, Throwable t) {
    if(t instanceof GemFireSecurityException) {
      securityLogger.info(LocalizedStrings.TWO_ARG_COLON, 
            new Object[] {message.toLocalizedString(), t}, t);
    } else {
      logger.info(LocalizedStrings.TWO_ARG_COLON,
         new Object[] {message.toLocalizedString(), t}, t);
    }
  }
