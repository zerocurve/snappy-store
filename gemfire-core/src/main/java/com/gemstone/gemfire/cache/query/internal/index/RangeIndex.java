...
public class RangeIndex extends AbstractIndex  {
  ...
  final ConcurrentSkipListMap<Object, Object> valueToEntriesMap =
      new ConcurrentSkipListMap<Object, Object>(
          TypeUtils.getExtendedNumericComparator());
  ...
  private void evaluate(Object key, int operator, Collection results,
    ...
    try {
      ...
      case OQLLexerTokenTypes.TOK_EQ: {
        ...
        addValuesToResult(this.valueToEntriesMap.get(key), results, limit,
            multiColOrderBy, context);
        break;
      }
      case OQLLexerTokenTypes.TOK_LT: {
        ConcurrentNavigableMap<Object, Object> sm = this.valueToEntriesMap
            .headMap(key, false);
        if (!asc) {
          sm = sm.descendingMap();
        }
        ...
        if (keysToRemove == null) {
          addValuesToResult(sm, results, limit, multiColOrderBy, context);
        } else {
          addValuesToResult(sm, results, keysToRemove, limit, multiColOrderBy, context);
        }
        break;
      }
      case OQLLexerTokenTypes.TOK_LE: {
      ...
      case OQLLexerTokenTypes.TOK_NE: {
        SortedMap sm = asc ? this.valueToEntriesMap : this.valueToEntriesMap
            .descendingMap();
        if (keysToRemove == null) {
          addValuesToResultSingleKeyToRemove(sm, results, key, limit,
              multiColOrderBy, context);
        } else {
...
try {
  switch (operator) {
  case OQLLexerTokenTypes.TOK_EQ: {
    addValuesToResult(this.valueToEntriesMap.get(key), results, iterOps,
        runtimeItr, context, projAttrib, intermediateResults, isIntersection,limit, multiColOrderBy);
    break;
  }  
  case OQLLexerTokenTypes.TOK_LT: {
    ConcurrentNavigableMap<Object, Object> sm = this.valueToEntriesMap
    ...
    addValuesToResult(sm, results,
        iterOps, runtimeItr, context, projAttrib, intermediateResults,isIntersection,limit, multiColOrderBy);
  ...
  case OQLLexerTokenTypes.TOK_NE_ALT:
  case OQLLexerTokenTypes.TOK_NE: {
    final SortedMap sm = asc ? this.valueToEntriesMap
        : this.valueToEntriesMap.descendingMap();
  ...
  if (operator == OQLLexerTokenTypes.TOK_EQ) { // result is empty
    ...
    final SortedMap sm = asc ? this.valueToEntriesMap
        : this.valueToEntriesMap.descendingMap();
    addValuesToResult(sm, results, iterOps, runtimeItr,
        context, projAttrib,intermediateResults,isIntersection,limit,multiColOrderBy);
    nullMappedEntries.addValuesToCollection(results, iterOps, runtimeItr,
        context, projAttrib,intermediateResults,isIntersection,limit);
    undefinedMappedEntries.addValuesToCollection(results, iterOps,
        runtimeItr, context, projAttrib, intermediateResults,isIntersection,limit);
...
private void addValuesToResult(Object entriesMap, Collection result, int limit, 
    boolean multiColOrderBy, ExecutionContext context) {
  ...
  if (entriesMap instanceof SortedMap) {
    ...
    Iterator entriesIter = sortedMap.values().iterator();
    ...
    while (entriesIter.hasNext()) {
      ...
      RegionEntryToValuesMap rvMap = (RegionEntryToValuesMap) entriesIter
          .next();
      rvMap.addValuesToCollection(result,multiColOrderBy?-1:limit, context);
      if (verifyLimit(result, limit, context)) {
        observer.limitAppliedAtIndexLevel(this, limit,result);
        return;
      }
...
private void addValuesToResult(Object entriesMap, Collection result,
    CompiledValue iterOps, RuntimeIterator runtimeItr,
    ExecutionContext context, List projAttrib, SelectResults intemediateResults, 
    boolean isIntersection,int limit, boolean multiColOrderBy )
...
//  Object orderByAttrs = context.cacheGet(CompiledValue.ORDERBY_ATTRIB);
//  int limit = -1;
  ...
  if (entriesMap instanceof SortedMap) {
    ...
    while (entriesIter.hasNext()) {
      ...
      rvMap.addValuesToCollection(result, iterOps, runtimeItr, context,
          projAttrib,intemediateResults,isIntersection,multiColOrderBy?-1:limit );
 ...
 */
private void addValuesToResult(Object entriesMap, Collection result,
    Set keysToRemove, int limit, boolean multiColOrderBy, ExecutionContext context) {
...
private void addValuesToResultSingleKeyToRemove(Object entriesMap, Collection result,
    Object keyToRemove, int limit, boolean multiColOrderBy, ExecutionContext context) {
  if (entriesMap == null || result == null)
    return;
  QueryObserver observer = QueryObserverHolder.getInstance();
  if (verifyLimit(result, limit, context)) {
...
void lockedQuery(Object key, int operator, Collection results, Set keysToRemove, ExecutionContext context)
  ...
  //Uncomment once we figure out how to apply limit on range index correctly
  //This is for bug45051
//  Boolean applyLimit = (Boolean)context.cacheGet(CompiledValue.CAN_APPLY_LIMIT_AT_INDEX); 
//  if(applyLimit != null && applyLimit.booleanValue()) {
//    limit = ((Integer)context.cacheGet(CompiledValue.RESULT_LIMIT)).intValue();
//  }
  ...
  if (key == null) {
    switch (operator) {
      case OQLLexerTokenTypes.TOK_EQ: {
        nullMappedEntries.addValuesToCollection(results,multiColOrderBy?-1:limit, context);
        break;
      }
      case OQLLexerTokenTypes.TOK_NE_ALT:
      case OQLLexerTokenTypes.TOK_NE: { // add all btree values        
...
//    }
    ...
    ConcurrentNavigableMap<Object, Object> dataset = this.valueToEntriesMap
        .subMap(lowerBoundKey, lowerBoundInclusive, upperBoundKey,
            upperBoundInclusive);
    if (!asc) {
      dataset = dataset.descendingMap();
    }
    //Object lowerBoundKeyToRemove = null;
    if (keysToRemove != null) {
      addValuesToResult(dataset, results, keysToRemove, limit, multiColOrderBy, context);
    }
    else {
      addValuesToResult(dataset, results, limit,  multiColOrderBy, context);
    }
  }
