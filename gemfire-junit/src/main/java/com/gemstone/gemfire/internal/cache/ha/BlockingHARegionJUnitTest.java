 ...
 */
  public void testBoundedPuts()
  {
    try {
      ...
      DistributedTestBase.join(thread1, 30 * 1000, null);
      DistributedTestBase.join(thread2, 30 * 1000, null);
  ...
  public void testPutBeingBlocked()
  {
    try {
      ...
      DistributedTestBase.waitForCriterion(ev, 3 * 1000, 200, true);
      ...
      DistributedTestBase.waitForCriterion(ev, 30 * 1000, 1000, true);
      
      DistributedTestBase.join(thread1, 30 * 1000, null); // for completeness
      DistributedTestBase.join(thread2, 30 * 1000, null);
      if (exceptionOccured) {
        fail(" Test failed due to " + exceptionString);
      }
  ...
  public void _testConcurrentPutsTakesNotExceedingLimit()
  {
    try {
      ...
      DistributedTestBase.join(thread6, 30 * 1000, null);
      DistributedTestBase.join(thread7, 30 * 1000, null);
      DistributedTestBase.join(thread8, 30 * 1000, null);
      DistributedTestBase.join(thread9, 30 * 1000, null);
      DistributedTestBase.join(thread10, 30 * 1000, null);
      ...
      DistributedTestBase.join(thread1, 30 * 1000, null);
      DistributedTestBase.join(thread2, 30 * 1000, null);
      DistributedTestBase.join(thread3, 30 * 1000, null);
      DistributedTestBase.join(thread4, 30 * 1000, null);
      DistributedTestBase.join(thread5, 30 * 1000, null);
